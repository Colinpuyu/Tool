...
const InputForm: React.FC<InputFormProps> = ({ onAdd, rules }) => {
  ...
  // Select top 4 rules for preview card (excluding trivial ones if needed, but taking first 4 for now)
  // Or prioritize rules with 'plate', 'pole' keywords if they exist, else just take first 4.
    ...
    <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 h-fit sticky top-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              长 (m) <span className="text-gray-400 text-xs font-normal">[L]</span>
            </label>
            <input
              type="number"
              min="0"
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              宽 (m) <span className="text-gray-400 text-xs font-normal">[W]</span>
            </label>
            <input
              type="number"
              min="0"
        ...
        {mainRule && (
          <div className="mt-6 bg-gradient-to-br from-blue-50 to-indigo-50 p-5 rounded-xl border border-blue-100 shadow-sm">
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-blue-900 font-bold text-sm uppercase tracking-wide">预览: {mainRule.label}</h3>
              <span className="text-2xl font-bold text-blue-600 tabular-nums">{previewResult[mainRule.id]}</span>
            </div>
            <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs text-gray-600 mb-4">
              {subRules.map(rule => (
                <div key={rule.id} className="flex justify-between border-b border-blue-100 pb-1">
            ...
            <button
              type="submit"
              className="w-full bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-bold py-2.5 px-4 rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 group"
            >
